...
for det in dets:
    ...
    util.plot_functions(time, timemax, SNR, pcolor, eventname, det, plottype, tevent, strain_whitenbp, template_match, template_fft,datafreq, d_eff, freqs, data_psd, fs)
...
# write the files:
util.write_wavfile('audio/' + eventname+"_H1_whitenbp.wav",int(fs), strain_H1_whitenbp[indxd])
util.write_wavfile('audio/' + eventname+"_L1_whitenbp.wav",int(fs), strain_L1_whitenbp[indxd])
...
util.write_wavfile('audio/' + eventname+"_template_whiten.wav",int(fs), template_p_smooth[indxt])
...
# write the files:
util.write_wavfile('audio/' + eventname+"_H1_shifted.wav",int(fs), strain_H1_shifted[indxd])
util.write_wavfile('audio/' + eventname+"_L1_shifted.wav",int(fs), strain_L1_shifted[indxd])

# and the template:
template_p_shifted = util.reqshift(template_p_smooth,fshift=fshift,sample_rate=fs)
util.write_wavfile('audio/' + eventname+"_template_shifted.wav",int(fs), template_p_shifted[indxt])
